        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if use_cache:
            if past_key_values is None:
                ...
                return_legacy_cache = True
                # logger.warning(
                #     "Passing tuple-based `past_key_values` is deprecated. "
                #     "Please wrap it with `DynamicCache.from_legacy_cache`."
                # )
        ...
        if use_cache:
            if return_legacy_cache and past_key_values is not None:
                next_past = past_key_values.to_legacy_cache()
            else:
                next_past = past_key_values
    ...
    def forward(
        ...
        if pixel_values.dim() == 5:
            ...
            if interpolate_pos_encoding and image_grid_thw is not None:
                # breakpoint()
                flatten_image_grid_thw = image_grid_thw.flatten()
                # print("flatten_image_grid_thw: ", flatten_image_grid_thw)
                assert batch_size == 1
                start = 0
...
class PaddleOCRVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        # print("vision_return_embed_list: ", vision_return_embed_list)
        # print("return_pooler_output: ", return_pooler_output)
        # print("use_rope: ", use_rope)
...
class PaddleOCRVLLMHead(nn.Module):
    def __init__(
        ...
        self.config = config if config is not None else decoder.config
...
class PaddleOCRVLForConditionalGeneration(Ernie4_5PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.lm_head_module = PaddleOCRVLLMHead(self.model, self.lm_head, config)
    ...
    def forward(
        ...
        **kwargs,
        ...
        return_outputs = self.lm_head_module(
            inputs_embeds=inputs_embeds,
            position_ids=position_ids,
            attention_mask=attention_mask,
            past_key_values=past_key_values,
        ...
        return PaddleOCRVLCausalLMOutputWithPast(
            logits=return_outputs[0],
            past_key_values=return_outputs[1],
