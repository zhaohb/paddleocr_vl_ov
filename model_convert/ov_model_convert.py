...
示例用法:
  ...
  # 使用CPU设备并启用INT8压缩
  python ov_model_convert.py --device cpu --llm_int8_compress --vision_int8_quant

  # 使用CPU设备并启用INT4压缩（更小的模型，但可能影响精度）
  python ov_model_convert.py --device cpu --llm_int4_compress --vision_int8_quant

  # 同时启用INT4和INT8压缩（生成两个压缩模型）
  python ov_model_convert.py --device cpu --llm_int4_compress --llm_int8_compress --vision_int8_quant
...
注意:
  ...
  - 当前脚本仅支持 CPU
    ...
    parser.add_argument(
        ...
        default="cpu",
        choices=["cpu"],
        help="OpenVINO编译使用的设备（当前仅支持 CPU） (默认: %(default)s)",
    )
    parser.add_argument(
        "--llm_int4_compress",
        action="store_true",
